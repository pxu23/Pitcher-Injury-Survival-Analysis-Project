# create the X, T, E for the pitcher
import pandas as pd
import torch
...
def prepare_time_varying_pitcher_input_for_rnn(pitcher_name):
        """
            ...
            :param player_name: the player name that the RNN model is making a prediction for
        ...
        # events (whether an injury occured)
        # features (workload and pitcher demographics) before each injury recurrence
        X, T, E = [], [], []
        ...
        for idx, row in injury_recurrence_pitcher.iterrows():
            ...
            # STEP 3: get the corresponding game rows in the survival dataframe for the pitcher before the injury
            ...
            features = survival_df_time_varying_recurrence_pitcher.drop(columns=['player_name', 'game_date',
                                                                   'start', 'stop', 'event', 'Batting Hand',
                                                                   'Throwing Hand', 'release_spin_rate_10_percentile',
            ...
            # STEP 5: Get the number of games played before injury
        ...
        # STEP 7: Pad the sequence of variable legnth sequence of features with zeros
